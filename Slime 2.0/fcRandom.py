...
def RD(Range, Num, algorithm, filterRule, filterPath):
    ...
    if filterRule == 'T':  # 过滤√
        ...
        try:
            ...
            resultLst_out.extend(randomSample(filter_numLst, k=Num))

            logger.debug('[算法]%s-[过滤规则路径]%s-[结果]%s' %
                         (algorithm, filterPath, sorted(resultLst_out)), 'fcRandom.py')
        ...
        try:
            ...
            resultLst_out.extend(randomSample(numLst, k=Num))
            logger.debug('[算法]%s-[结果]%s' %
                         (algorithm, sorted(resultLst_out)), 'fcRandom.py')
